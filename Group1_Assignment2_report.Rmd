...
round(exp(lr_final_resuts$coefficients[,1]),1)
```
...
```{r}
rbind(
  perf.metrics(lr,  test, best_formula, threshold = 0.5, model.name = "Logistic Regression", lr = TRUE),
  perf.metrics(dt,  test, best_formula, threshold = 0.5, model.name = "Decision Tree"),
  perf.metrics(rf,  test, best_formula, threshold = 0.5, model.name = "Random Forest"),
  perf.metrics(xgb, test, best_formula, threshold = 0.5, model.name = "XGBoost", xgboost = TRUE)
) %>% 
