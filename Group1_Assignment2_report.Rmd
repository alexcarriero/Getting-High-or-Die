...
```{r}
# Choose best formula from logistic regression 
...
```{r}
# Random Forest
rf <- randomForest(class ~ ., beginner_train)
...
perf.metrics <- function(model, test, best_formula, threshold = 0.5, model.name = NA, xgboost = FALSE) {
  ...
  if (xgboost) {
    ...
    pred_class <- ifelse(
      predict(model, newdata = test) < threshold, 0, 1)
  }
...
```{r}
predict(rf, type = "vote", newdata = beginner_test)
```

```{r}
perf.metrics(rf, beginner_test, best_formula, threshold = 0.5, model.name = NA, xgboost = FALSE)
```

```{r}
table(true = beginner_test$class, predicted = ifelse(lr_prob > 0.5, 1, 0))
```
