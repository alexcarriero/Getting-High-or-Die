...
In order to not rely on chance in our selection we applied 5-fold cross-validation and averaged the results over the folds. In this way we obtained the best subset of predictors as well as the corresponding threshold value based on the training set. We use the baseline threshold in all models

Next we applied cross-validation again to compare the performance first on the training set between the different candidate models and also evaluated the performance on the test set. For these comparisons we took the our chosen metric into account.
...
grViz(diagram = "digraph flowchart {
  ...
  [4]: 'The mean of the metrics over these 5 models is caculated'
  [5]: 'The best forumla is selected based on the highest mean'
")
```
...
In Table 2 we present winning combinations of predictors. The combination of predictors with the highest weighted balanced accuracy across all levels of predictors is 6 predictors. However, the difference between 5 and 6 is so verylow. Since we would like to keep the model as simple as possible so that even beginners can eat mushrooms, we chose the model with the best combination of 5 predictors as our final set of predictors for simplicity reasons.
...
A benefit of performing logistic regression is that parameter estimates are interpretable  **The final logistic model is:** class ~ cap.shape + bruises + stalk.shape + ring.number + habitat. The output of the model, can be seen below. There are insignificant predictors in all predictors, this can mainly be explained by the small number of observations in these predictor groups. The significant predictors will be discussed.
...
stargazer(lr_final, odd.ratio = T, type = "html",
          dep.var.labels = "Posionious mushroom",
...
```{r}
tibble(
  var = rownames(rf_importance),
  imp = rf_importance
)%>%
  ggplot(aes(x = var, y = rf_importance)) +
  geom_bar(stat = "identity", width = 0.05, fill = "skyblue")+
  geom_point( col = "lightsalmon2", size = 3) + 
  coord_flip() + 
  xlab("Importance") +
  ylab("Predictor") +
  ggtitle("Importance of predictors from random forest model") + 
  theme_minimal()
```
...
**something something about the others having interpretability but xgboost has best performance**