...
For these comparisons we took the accuracy and our chosen metric into account.
...
grViz(diagram = "digraph flowchart {
  ...
  [2]: 'Run a LR for all formulas'  
  [3]: 'Obtain metrics for different thresholds'  
  [4]: 'Take the mean of the best metrics for each formula'
")
```
...
find_best_predictors <- function(formulas, train, valid, valid_y){
  ...
  for(i in 1:length(formulas)){
    ...
    for(j in 1:nrow(thres)){
      ...
      comb[j,1] <- TP / (TP + FN)  # sensitivity 
      comb[j,2] <- TN / (TN + FP)  # specificity
      comb[j,3] <- (comb[j,1]+comb[j,2]*1.5)/2.5
      }
...
perf.metrics <- function(model, test, best_formula, threshold = 0.5, model.name = NA, lr = FALSE, xgboost = FALSE) {
...
We assess model performance across three categories: accuracy, discrimination and calibration. Accuracy is assessed using total accuracy and weighted balanced accuracy (Introduced in Section 2.1).  

- Accuracy: $\frac{\mathrm{TP} + \mathrm{TN}}{N}$
- Weighted Balanced Accuracy:  $\frac{1.5*Sp + Sn}{2.5}$

In these formulae, TP reflects the number of true positives, TN reflects the number of true negatives, N is the total number of observations in the sample, Sn represents sensitivity and Sp represents specificty.  Discrimination is assessed via area under the receiver operator curve (AUROC).  Finally, we assess calibration visually by means of flexible calibration curves. 
...
rbind(pred2, pred3, pred4, pred5, pred6) %>%
  ...
  mutate(lower_bound = mean_performance - sd_performance) %>% 
  kbl(format = "html", caption = "Performance of the best sets of predictors.") %>% 
  kable_styling("striped") 
# not a huge improvement between 3, 4, and 5
# All of these models would yield good results
```

The lower bound in the table above refers to a lower estimation of model performance for the given number of predictors. If this lower bound is higher than the mean performance for the models with one less predictor, we estimate that the gain from adding one predictor is important. There is a definite improvement from 2 to 3 predictors and from 3 to 4 predictors, but not from 4 to 5. We therefore consider models with 4 to 6 predictors to perform equally well and perform cross-validation on the best of each model.\
...
res %>%
  rename("Sensitivity" = CV_TPR, 
         "Specificity" = CV_TNR, 
         "Accuracy"    = CV_Metric)%>%
  mutate(Formula = c(pred4[1,1], pred5[1,1], pred6[1,1]), 
         DT      = c(pred4[1,3], pred5[1,3], pred6[1,3])) %>% 
  dplyr::select(Formula, DT, Sensitivity, Specificity, Accuracy) %>%
  kbl(format = "html", caption = "Performance of the best set of predictors for 4, 5, 6 predictors.") %>% 
  kable_styling("striped") 
```

**The final model is:** class ~ cap.shape + bruises + stalk.shape + ring.number + habitat. The output of the model, as can be seen above. There are insignificant predictors in all predictors, this can mainly be explained by the small number of observations in these predictor groups. The significant predictors will be discussed.
...
stargazer(lr_final, odd.ratio = T, type = "text", single.row = TRUE)
```

The output of logistic regression is the log odds ratio, this is a metric which is hard to interpret. The odds ratio (OR) is easier to interpret and can be obtained by taking the exponent of the log odds ratio. If the odds ratio is larger (lower) than 1 then that group is associated with higher (lower) odds of the outcome.
...
table3 <- tibble(
  ...
  `Catogorie` = c("bell", "flat", "knobbed", "false", "tapering", " two", "grasses", "woods"),
...
table3 %>%
  ...
  kable_styling("striped") 
```
As can be seen from the table above if a mushroom has a cap-shaped flat/knobbed or there are no bruises then the mushroom is associated with lower odds of being poisonous compared to their baseline groups. For mushrooms with bell cap shapes, stalk shapes that tare tapering, two rings and live in a grass or woods habitat they are associated with higher odds of being poisonous compared to their baseline groups.
...
xgb <- xgboost(
  ...
  verbose = 2)
# TODO: need to make sure nothing prints from this chunk
```
...
```{r}
rbind(
  perf.metrics(lr,  test, best_formula, threshold = 0.48, model.name = "Logistic Regression", lr = TRUE),
  perf.metrics(dt,  test, best_formula, threshold = 0.48, model.name = "Decision Tree"),
  perf.metrics(rf,  test, best_formula, threshold = 0.48, model.name = "Random Forest"),
  perf.metrics(xgb, test, best_formula, threshold = 0.48, model.name = "XGBoost", xgboost = TRUE)
) %>% 
...
```{r}
table(beginner_train$class)
3361 /(3138 +3361 )
```


The table above shows the performance of all types of models for the best set of predictors. It appears XGBoost was the best model on all metrics of interest.
...
probs %>%
  ...
  ggplot(aes(x = value, y = class, color = variable, fill = variable)) + 
  geom_abline(slope = 1, intercept = 0, size = 1) +
  geom_smooth(method = stats::loess, se = TRUE) +
  scale_x_continuous(breaks = seq(0, 1, 0.1)) +
  scale_y_continuous(breaks = seq(0, 1, 0.1)) +
  # scale_color_discrete(labels=c('Decision Tree', 'Logistic Regression', 'Random Forest', 'XGBoost'))+
  scale_fill_brewer(palette = "Set2") +
  scale_color_brewer(palette = "Set2") +
  xlab("Estimated Probability") +
  ylab("Observed Proportion") +
  labs(color = "Model", fill = "Model") +
  ggtitle("Flexible Model Calibration") + 
  theme_minimal()
```

Additionally, we considered calibration. It appears logistic regression and random forest were severely miscalibrated, while decision trees and XGboost tended to underestimate the probability of edibility. Underestimating edibility is more desirable than overestimating it, so we do not consider the slight underestimation of XGBoost to be a problem.
...
We aimed to identify a model predicting mushroom edibility. We performed logistic regression to identify the best set of predictors, then compared the performance of logistic regression with tree-based methods. It appeared that XGBoost performed the best on the metrics of interest; additionally, it slightly underestimated the probability of a mushroom being edible, while other models were severely miscalibrated. We thus conclude that XGBoost is the best model.
